...
# Works both via `curl | bash` (downloads from GitHub) and when run locally
# (reads files from the repo checkout).
set -euo pipefail

PLUGIN_NAME="opencode-ollama-websearch"
SKILL_NAME="using-ollama-web-search"
REPO_RAW="https://raw.githubusercontent.com/justin-carper/opencode-ollama-websearch/main"
CONFIG_DIR="${OPENCODE_CONFIG_DIR:-${HOME}/.config/opencode}"
SKILL_DST="${HOME}/.agents/skills/$SKILL_NAME"
CMD_DST="$CONFIG_DIR/commands"

# Determine script source: local repo checkout or piped from curl
...
# Check for curl (needed when running from curl, or if downloading files)
NEED_CURL=false
if [ -z "$LOCAL_REPO" ]; then
  NEED_CURL=true
  if ! command -v curl >/dev/null 2>&1; then
    echo "Error: curl is required when running via curl|bash."
    exit 1
  fi
fi
...
if [ -n "$LOCAL_REPO" ]; then
  cp -R "$LOCAL_REPO/skill/$SKILL_NAME/." "$SKILL_DST/"
else
  mkdir -p "$SKILL_DST"
  curl -fsSL "$REPO_RAW/skill/$SKILL_NAME/SKILL.md" -o "$SKILL_DST/SKILL.md"
  curl -fsSL "$REPO_RAW/skill/$SKILL_NAME/reference.md" -o "$SKILL_DST/reference.md"
fi
echo "[skill]  installed -> $SKILL_DST"

# --- 3. Install the slash commands ---
if [ -n "$LOCAL_REPO" ]; then
  cp "$LOCAL_REPO/commands/ollama-search.md" "$CMD_DST/"
  cp "$LOCAL_REPO/commands/ollama-fetch.md" "$CMD_DST/"
else
  curl -fsSL "$REPO_RAW/commands/ollama-search.md" -o "$CMD_DST/ollama-search.md"
  curl -fsSL "$REPO_RAW/commands/ollama-fetch.md" -o "$CMD_DST/ollama-fetch.md"
fi
